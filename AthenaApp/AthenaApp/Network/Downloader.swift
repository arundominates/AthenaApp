...
class Downloader: Operation {
    ...
    override func main() {
        ...
        self.executing(true)
        self.downloadImageFromUrl(resize: false, width: 150, height: 150)
    }
    
    func downloadImageFromUrl(resize: Bool,width: Int, height: Int) {
        ...
        let downloadTask = newSession.downloadTask(with: self.imageUrl) { (location, response, error) in
            if let locationUrl = location, let data = try? Data(contentsOf: locationUrl){
                ...
                if resize {
                    guard let imageResized = UIImage(data: data)?.resized(to: CGSize(width: width, height: height)) else { return
                    }
                    image = imageResized
                } else {
